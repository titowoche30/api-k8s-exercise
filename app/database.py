...
class Database:
    ...
    def create_database_if_not_exists(self, connection, database_name='fastapi'):
        if not self.check_if_database_exists(connection, database_name):
            self.create_database(connection, database_name)
        else:
    ...
    def execute_database_startup(self):
        postgres_conn = self.get_connection()
        self.create_database_if_not_exists(postgres_conn, POSTGRES_DB)
